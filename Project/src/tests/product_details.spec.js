...
test("check product card", async ({ page }) => {
  ...
  await test.step("Add quantity of product", async () => {
    ...
    // Now:
    //
    // ProductPage handles this action
  });
  ...
  await test.step("User write a review", async () => {
    ...
    // Before:
    //
    // await productPage.yourNameInput.fill()
    // await productPage.emailAddressInput.fill()
    // await productPage.addReviewHereInput.fill()
    // await productPage.messageSubmitButton.click()
    //
    // Now:
    //
    // One business action = one method
  });
});
